...
void main() {
  testWidgets('Customers tab rendering shows list item', (tester) async {
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          sessionUserProvider.overrideWith((ref) => Stream.value(_user('owner'))),
          customersListProvider.overrideWith((ref, tag) => Stream.value([
            _customer(),
          ])),
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            sessionUserProvider.overrideWith((ref) => Stream.value(_user('owner'))),
            customersListProvider.overrideWith(
              (ref, tag) => Stream<List<Customer>>.error(Exception('load failed')),
            ),
            customerSearchProvider.overrideWith(
              (ref, query) => Future.value(<Customer>[]),
            ),
  ...
  testWidgets(
    'slow network simulation keeps loading then renders data',
    (tester) async {
